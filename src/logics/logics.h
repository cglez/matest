...
struct _LlUCon
{
	char   symbol_pn[SLEN + 1];  /**< Símbolo que representa la conectiva (una letra mayúscula en la notación polaca). */
	char   symbol[SLEN + 1];     /**< Símbolo de la conectiva en notación estándar. */
	int    *matrix;              /**< Matriz de la conectiva: una matriz línea que define la semántica de la conectiva;
	                                  contiene los valores que ésta asignará a una variable. */
};
...
struct _LlBCon
{
	char   symbol_pn[SLEN + 1]; /**< Símbolo que representa la conectiva en notación polaca (una letra mayúscula). */
	char   symbol[SLEN + 1];    /**< Símbolo de la conectiva en notación estándar. */
	int    **matrix;            /**< Matriz de la conectiva: una matriz cuadrada que define la semántica de la conectiva;
	                                 contiene los valores que ésta asignará a un par de variables. */
};
...
struct _LlWFF
{
	...
	char         symbol[SLEN + 1];    /**< Símbolo que lo representa.       */
...
gint          ll_wff_parse_formula              (LlWFF         **wff,
                                                 char          *formula,
                                                 LlLogic       *logic);
LlSymbolType  ll_symbol_type                    (const char    *symbol,
                                                 LlLogic       *logic);

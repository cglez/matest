 ...
 */
char
readin (const char *str, const char *pattern)
{
	char ch;
	int i;
...
// FIXME Usar nombre de archivo en vez del descriptor
				...
				if (isspace (auxchar)) {
					if (mtx) {
						if (mtx[1][0] == -1 && con_symbol != 0) {
							...
							ucon = ll_ucon_new (strsymb, strsymb, mtx[0], logic->dimension);
							...
							sprintf (strsymb, "%c", con_symbol);
							bcon = ll_bcon_new (strsymb, strsymb, mtx, logic->dimension);
